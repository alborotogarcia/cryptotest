...
export const CryptoNewItem = ({ cryptoNew }: Props) => {
    return (
        <Flex boxShadow={['sm', 'md', 'lg', 'xl']} p={3} bg="white" borderRadius={['sm', 'md', 'lg', 'xl']} maxWidth="100%" >
            <Flex flexDirection="column">
                {/* <Flex direction={{ base: 'column-reverse', md: 'row' }}> */}
                <Flex flexDirection="row">
                    <Badge h="25%" bg="yellow" borderRadius="full">
                        <Text>{cryptoNew.kind}</Text>
                    </Badge>
                    <Badge  h="25%" bg="orange" borderRadius="full">
                        <Text fontWeight="bold">{cryptoNew.domain}</Text>
                    </Badge>
                    <Badge  h="25%" bg="cyan" borderRadius="full">
                        {cryptoNew.sentiment}
                    </Badge>
                </Flex>
